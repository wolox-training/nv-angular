angular.module('store').controller('DashboardController', [
  function () {
    const dashboard = this;    
    dashboard.products = books;
    dashboard.filteredProducts = dashboard.products;
        
    dashboard.searchBook = function() {
      if(dashboard.searchBox && dashboard.filterValue) {
        dashboard.filteredProducts = dashboard.products.filter(book => book[dashboard.filterValue].toLowerCase().indexOf(dashboard.searchBox.toLowerCase()) > -1);
      } else {
        dashboard.filteredProducts = dashboard.products;     
      }
    };
